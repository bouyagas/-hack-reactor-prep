const aSequenceOfObjectInAnArray = (myArray) => {
  let obj = {};
  for (var i = 0; i < myArray.length; i++) {
     obj[myArray[i]] = myArray[myArray.length-1];
  }
  return obj;
};

console.log(aSequenceOfObjectInAnArray(['Kevin', 'Bacon', 'Love', 'Hart', 'Costner', 'Spacey']));
...
// Write a function that determines the area of a circle given the radius
 ...
 function findTheAreaOfACircle(radius) {
   var result = Math.pow(radius, 2) * Math.PI;
   return result;
 }

 findTheAreaOfACircle(5);
...
function countWords(str) {
  ...
  if (str !== "") {
    var arr = str.split(" ");
    for (var i=0; i<arr.length; i++) {
      if (typeof wordObj[arr[i]] != "undefined") {
      wordObj[arr[i]]++;
} else {
  wordObj[arr[i]] = 1;
}
